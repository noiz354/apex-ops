...
export function getVapidKeys(): { publicKey: string; privateKey: string } {
  ...
  if (!_keys) {
    ...
    console.error(`[push] EPHEMERAL-DEV-ONLY VAPID keys — set ${ENV_PUBLIC}/WEBPUSH_PRIVATE_VAPID_KEY for stability; subs will NOT survive restart`);
  }
  return _keys;
}
...
export async function sendP1PushToUser(
  ...
  if (!isPushConfigured()) {
    console.error('[push] P1 push SKIPPED — VAPID keys not configured; delivery impossible until env keys are set');
